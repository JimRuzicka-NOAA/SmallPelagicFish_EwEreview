function EcotrophicEfficiency = f_calcEE_05122022(randomEwE)
...
%                   5/12/2022 fix div/0 error NaNs for groups with 0 production
...
ee_eggs(isnan(ee_eggs))             = 0; % fix div/0 error for group with no production
ee_predation(isnan(ee_predation))	= 0; % fix div/0 error for group with no production
ee_ba(isnan(ee_ba))                 = 0; % fix div/0 error for group with no production
ee_em(isnan(ee_em))                 = 0; % fix div/0 error for group with no production
