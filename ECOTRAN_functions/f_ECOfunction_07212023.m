function [SINGLE_ECO] = f_ECOfunction_07212023(ModelDefinitions, CurrentModel)
...
%       f_calcEE_05122022                       calculate Ecotrophic Efficiency 
...
%       5/12/2022 updated to f_calcEE_05122022
%       5/13/2022 cleared up NaNs in ProductionBudget, ConsumptionBudget, & TransferEfficiency (due to ee_eggs for some groups with 0 consumption or catch)
%       7/21/2023 calculations with and without cannibalism (necessary for Mixed Trophic Impact)
...
consumption_domestic                            = sum(CONSUMPTION, 1);             	 % NEW total consumption BY each consumer c; includes cannibalism; (t/km2/yr); (horizontal vector: num_grps X 1)
...
EcotrophicEfficiency                = f_calcEE_05122022(randomEwE);
...
ee                                  = EcotrophicEfficiency.ee;              % fraction of all production used within food web and NOT senescence; (vertical vector: num_grps X 1); NOTE: in EwE this traditionally did not include em but it does in ECOTRAN
ee_predation                        = EcotrophicEfficiency.ee_predation;	% predation as proportion of production budget; (vertical vector: num_grps X 1)
ee_senescence                       = (1 - ee);                             % senescence; (vertical vector: num_grps X 1)
                                                                          	...
                                                                          	%   NOTE: use of ee that includes cannibalism in ee_predation to calculate ee_senescence
...
EcotrophicEfficiency_NoCannibalism	= f_calcEE_05122022(randomEwE);
ee_predation_NoCannibalism          = EcotrophicEfficiency_NoCannibalism.ee_predation;	% predation as proportion of production budget; (vertical vector: num_grps X 1)
% -------------------------------------------------------------------------
...
ee_predation_NoCannibalism(looky_NO3)	= round2(sum(PhytoUptake_NO3), 7);  % NOTE use of round2 here because of rounding error in the 10th decimal point
ee_predation_NoCannibalism(looky_NH4)	= round2([sum(PhytoUptake_NH4); sum(PhytoUptake_NH4)], 7); % FFF can treat pelagic & benthic pools separately; NOTE: use of round2 here because of rounding error in the 10th decimal point

ee_senescence(looky_NO3)                = 1 - (ee_predation(looky_NO3) + ee_ba(looky_NO3) + ee_em(looky_NO3));
ee_senescence(looky_NH4)                = 1 - (ee_predation(looky_NH4) + ee_ba(looky_NH4) + ee_em(looky_NH4));
% *************************************************************************
...
ee_eggs                 	= ee_senescence .* EwEegg_fraction';       % (vertical vector: num_grps X 1); NOTE transpose; NOTE: this line must come before the ee_senescence correction line
ee_senescence      = ee_senescence .* (1 - EwEegg_fraction)';	% (vertical vector: num_grps X 1); NOTE transpose
% -------------------------------------------------------------------------
...
%          NOTE: DECREASE ee_predation_NoCannibalism, ee_senescence, ee_ba, & ee_em by
%                the fraction of production going to "feces" eggs (distribute
%                this reduction based upon the fraction of production going to
%                each term (ee_predation_NoCannibalism, ee_senescence, ee_ba, or ee_em). The
...
ee_predation                = ee_predation                  - ((feces_EggCorrection./pq) .* ee_predation); 	% (vertical vector: num_grps X 1); cannibalism INCLUDED
ee_predation_NoCannibalism	= ee_predation_NoCannibalism	- ((feces_EggCorrection./pq) .* ee_predation_NoCannibalism); % (vertical vector: num_grps X 1); cannibalism REMOVED
ee_senescence               = ee_senescence                 - ((feces_EggCorrection./pq) .* ee_senescence);	% (vertical vector: num_grps X 1)
...
%          NOTE: do not decrease production going to ee_em, redistribute this egg adjustment of ee_em to ee_predation, ee_senescence, and ee_ba
em_EggCorrection            = (feces_EggCorrection./pq) .* ee_em;

temp_sum                    = ee_predation                  + ee_senescence + ee_ba;
temp_sum_NoCannibalism      = ee_predation_NoCannibalism	+ ee_senescence + ee_ba;

ee_predation                = ee_predation                  - (em_EggCorrection .* (ee_predation ./ temp_sum));
ee_predation_NoCannibalism	= ee_predation_NoCannibalism	- (em_EggCorrection .* (ee_predation_NoCannibalism ./ temp_sum_NoCannibalism));

ee_senescence               = ee_senescence                 - (em_EggCorrection .* (ee_senescence ./ temp_sum));
ee_ba                       = ee_ba                         - (em_EggCorrection .* (ee_ba      	  ./ temp_sum)); % QQQ change this to temp_sum_NoCannibalism??

ee_eggs(isnan(ee_eggs))                                         = 0; % set div/0 errors to 0
ee_predation(isnan(ee_predation))                               = 0; % set div/0 errors (e.g., fleets) to 0
ee_predation_NoCannibalism(isnan(ee_predation_NoCannibalism))	= 0; % set div/0 errors (e.g., fleets) to 0
ee_senescence(isnan(ee_senescence))                             = 0; % set div/0 errors (e.g., fleets) to 0
...
cb_production               = pq        .* (1 - temp_ee_em);    % fraction of consumption going to production; (vertical vector: num_grps X 1)
...
cb_predation_NoCannibalism	= pq .* ee_predation_NoCannibalism;	% fraction of consumption going to predation; (vertical vector: num_grps X 1)

cb_senescence               = pq .* ee_senescence;              % fraction of consumption going to senescence; (vertical vector: num_grps X 1)
...
cannibal_correction_cb   	= cb_predation - cb_predation_NoCannibalism; % portion of cb_predation due to cannibalism
...
cb_feces_NoCannibalism      = cb_feces      + (feces_fraction      .* cannibal_correction_cb);
cb_metabolism_NoCannibalism	= cb_metabolism + (metabolism_fraction .* cannibal_correction_cb);

% Reduce cb_production to account for cannibalism
cb_production_NoCannibalism	= cb_production - cannibal_correction_cb; % reduced to correct for cannibalism

% Reduce pq & pb to account for cannibalism
pq_NoCannibalism        	= cb_production_NoCannibalism; % pq_NoCannibalism is now modified to account for cannibalism
pb_NoCannibalism          	= pq_NoCannibalism .* qb;      % pb_NoCannibalism is now modified to account for cannibalism
% -------------------------------------------------------------------------


% step 6c: cannibalism corrections for ProductionBudget (ee) terms --------
cannibal_correction_ee      = cb_production ./ cb_production_NoCannibalism;
cannibal_correction_ee(isnan(cannibal_correction_ee)) = 0; % correct div/0 error (probably not necessary) 
ee_eggs_NoCannibalism       = ee_eggs                       .* cannibal_correction_ee; % eggs & reproduction; (vertical vector: num_grps X 1)
ee_predation_NoCannibalism	= ee_predation_NoCannibalism	.* cannibal_correction_ee;	% predation (M2_p / P_p); (vertical vector: num_grps X 1)
ee_senescence_NoCannibalism	= ee_senescence                 .* cannibal_correction_ee; % senescence (M0_p / P_p); fraction of production going to other mortality (non-consumed production); (vertical vector: num_grps X 1)
ee_ba_NoCannibalism       	= ee_ba                         .* cannibal_correction_ee; % ba (ba_p / P_p); (vertical vector: num_grps X 1)
ee_em_NoCannibalism        	= ee_em                         .* cannibal_correction_ee; % em (em_p / P_p); includes physical transport out of model domain; immigration is negative production, emigration is positive production; (vertical vector: num_grps X 1)
% -------------------------------------------------------------------------
...
%          NOTE: use ee_predation (predation that includes the cannibalism component) 
...
ee_NoCannibalism            =  ee_eggs_NoCannibalism + ee_predation_NoCannibalism + ee_ba_NoCannibalism + ee_em_NoCannibalism; % (vertical vector: num_grps X 1)
% -------------------------------------------------------------------------
...
TransferEfficiency_NoCannibalism	= ee_NoCannibalism .* pq_NoCannibalism; % (vertical vector: num_grps X 1)
% *************************************************************************
...
BioenergeticBudget_wCannibalism	= zeros(3, num_grps);
%                       1) feces
%                       2) metabolism
%                       3) production
ProductionBudget                = zeros(5, num_grps);
ProductionBudget_wCannibalism	= zeros(5, num_grps);
%                       1) eggs (reproduction)
%                       2) predation
%                       3) senescence
%                       4) ba (biomass accumulation)
...
ConsumptionBudget_wCannibalism	= zeros(7, num_grps);
%                       1) feces
%                       2) metabolism
%                       3) eggs (reproduction)
%                       4) predation
...
BioenergeticBudget(1, :)                = cb_feces_NoCannibalism';      % fraction of consumption going to feces; (horizontal vector: 1 X num_grps); NOTE transpose
BioenergeticBudget(2, :)                = cb_metabolism_NoCannibalism';	% fraction of consumption going to metabolism (NH4 production); Metabolism = 1 - PQ - feces = (1 - (EwE_PQ + (1-EwE_AE))); (horizontal vector: 1 X num_grps); NOTE transpose
BioenergeticBudget(3, :)                = cb_production_NoCannibalism';	% fraction of consumption going to production; (horizontal vector: 1 X num_grps); NOTE transpose

BioenergeticBudget_wCannibalism(1, :)	= cb_feces';        % fraction of consumption going to feces; (horizontal vector: 1 X num_grps); NOTE transpose
BioenergeticBudget_wCannibalism(2, :)	= cb_metabolism';	% fraction of consumption going to metabolism (NH4 production); Metabolism = 1 - PQ - feces = (1 - (EwE_PQ + (1-EwE_AE))); (horizontal vector: 1 X num_grps); NOTE transpose
BioenergeticBudget_wCannibalism(3, :)	= cb_production';	% fraction of consumption going to production; (horizontal vector: 1 X num_grps); NOTE transpose
% -------------------------------------------------------------------------
...
ProductionBudget(1, :)                  = ee_eggs_NoCannibalism';     	% eggs & reproduction; NOTE: transpose
ProductionBudget(2, :)                  = ee_predation_NoCannibalism';	% predation (M2_p / P_p); NOTE: transpose
ProductionBudget(3, :)                  = ee_senescence_NoCannibalism';	% senescence (M0_p / P_p); fraction of production going to other mortality (non-consumed production); NOTE: cannibalism REMOVED; prior to 12/29/2020 elimination of cannibalism predation resulted in increased senescence; NOTE: transpose
ProductionBudget(4, :)                  = ee_ba_NoCannibalism';        	% ba (ba_p / P_p); NOTE: transpose
ProductionBudget(5, :)                  = ee_em_NoCannibalism';       	% em (em_p / P_p); includes physical transport out of model domain; immigration is negative production, emigration is positive production; NOTE: transpose

ProductionBudget_wCannibalism(1, :)     = ee_eggs';                     % eggs & reproduction; NOTE: transpose
ProductionBudget_wCannibalism(2, :)     = ee_predation';                % predation (M2_p / P_p); NOTE: transpose
ProductionBudget_wCannibalism(3, :)     = ee_senescence';               % senescence (M0_p / P_p); fraction of production going to other mortality (non-consumed production); NOTE: cannibalism REMOVED; prior to 12/29/2020 elimination of cannibalism predation resulted in increased senescence; NOTE: transpose
ProductionBudget_wCannibalism(4, :)     = ee_ba';                       % ba (ba_p / P_p); NOTE: transpose
ProductionBudget_wCannibalism(5, :)     = ee_em';                       % em (em_p / P_p); includes physical transport out of model domain; immigration is negative production, emigration is positive production; NOTE: transpose
% -------------------------------------------------------------------------
...
ConsumptionBudget(1, :)                 = cb_feces_NoCannibalism';        % feces; (horizontal vector: 1 X num_grps); NOTE transpose
ConsumptionBudget(2, :)                 = cb_metabolism_NoCannibalism';	% metabolism; (horizontal vector: 1 X num_grps); NOTE transpose
...
ConsumptionBudget_wCannibalism(1, :)	= cb_feces';        % feces; (horizontal vector: 1 X num_grps); NOTE transpose
ConsumptionBudget_wCannibalism(2, :)   	= cb_metabolism';	% metabolism; (horizontal vector: 1 X num_grps); NOTE transpose
ConsumptionBudget_wCannibalism(3, :)  	= cb_eggs';         % eggs; (horizontal vector: 1 X num_grps); NOTE transpose
ConsumptionBudget_wCannibalism(4, :)   	= cb_predation';	% predation; (horizontal vector: 1 X num_grps); NOTE transpose
ConsumptionBudget_wCannibalism(5, :)   	= cb_senescence';	% senescence; (horizontal vector: 1 X num_grps); NOTE transpose
ConsumptionBudget_wCannibalism(6, :)   	= cb_ba';           % ba; (horizontal vector: 1 X num_grps); NOTE transpose
ConsumptionBudget_wCannibalism(7, :)   	= cb_em';           % em; (horizontal vector: 1 X num_grps); NOTE transpose
% -------------------------------------------------------------------------
...
PredationBudget_wCannibalism	= f_calcPredationBudget_12102019(consumption_domestic, DIET); % (2D matrix: num_grps X num_grps; consumers X producers)
% *************************************************************************
...
BigMatrix_wCannibalism                          = PredationBudget_wCannibalism; % initialize; (2D matrix: num_grps X num_grps)
% -------------------------------------------------------------------------
...
BigMatrix_wCannibalism(looky_ANYPrimaryProd, looky_NO3)     = repmat(PhytoUptake_NO3, [1, num_NO3]);            % distribute NO3 among primary producers
BigMatrix_wCannibalism(looky_ANYPrimaryProd, looky_plgcNH4)	= repmat(PhytoUptake_NH4, [1, num_plgcNH4]);        % distribute pelagic NH4 among primary producers; rescaled to allow NH4 oxidation to NO3
BigMatrix_wCannibalism(looky_ANYPrimaryProd, looky_bnthNH4)	= repmat(PhytoUptake_NH4, [1, num_bnthNH4]);        % distribute benthic NH4 among primary producers; rescaled to allow NH4 oxidation to NO3
% -------------------------------------------------------------------------
...
temp_cb_predation                                   = cb_predation_NoCannibalism; 
temp_cb_predation(looky_terminalBNTHdetritus)       = cb_predation_NoCannibalism(looky_terminalBNTHdetritus) + cb_senescence(looky_terminalBNTHdetritus); % special case for terminal benthic detritus group(s)
...
sum_predation                                       = sum(BigMatrix_wCannibalism(looky_livingANDfleets, :), 1);      % (horizontal vector: 1 X num_grps)
temp_livingANDfleets                                = BigMatrix_wCannibalism(looky_livingANDfleets, :) ./ repmat(sum_predation, [num_livingANDfleets, 1]); % temporarily renormalize predation rows to 1; (2D matrix: num_livingANDfleets X num_grps)
temp_livingANDfleets(isnan(temp_livingANDfleets))   = 0;                  % fix div/0 errors
...
temp_livingANDfleets                                = temp_livingANDfleets .* repmat(temp_cb_predation', [num_livingANDfleets, 1]); % rescale by predation term of ConsumptionBudget; (2D matrix: num_livingANDfleets X num_grps); NOTE transpose
BigMatrix_wCannibalism(looky_livingANDfleets, :)	= temp_livingANDfleets;                             % paste predator rows back into BigMatrix; (2D matrix: num_grps X num_grps)
% -------------------------------------------------------------------------
...
temp_eggs                                           = repmat(ConsumptionBudget_wCannibalism(3, :), [num_eggs, 1]);	% (2D matrix: num_eggs X num_grps)
temp_eggs                                           = temp_eggs .* fate_eggs;                           % redistribute among egg rows as defined by fate_eggs; (2D matrix: num_eggs X num_grps)
BigMatrix_wCannibalism(looky_eggs, :)               = temp_eggs;                                        % paste egg (& reproduction) rows back into BigMatrix; (2D matrix: num_grps X num_grps)
% -------------------------------------------------------------------------
...
temp_feces                                      = repmat(ConsumptionBudget_wCannibalism(1, :), [num_ANYdetritus, 1]);	% (2D matrix: num_ANYdetritus X num_grps)
temp_feces                                      = temp_feces .* fate_feces_unified;                         % redistribute feces among detritus rows as defined by fate_feces_unified; (2D matrix: num_ANYdetritus X num_grps)
temp_senescence                                 = repmat(ConsumptionBudget_wCannibalism(5, :), [num_ANYdetritus, 1]);    % (2D matrix: num_ANYdetritus X num_grps)
temp_senescence                                 = temp_senescence .* fate_senescence_unified;               % redistribute senescence among detritus rows as defined by fate_senescence_unified; (2D matrix: num_ANYdetritus X num_grps)
BigMatrix_wCannibalism(looky_ANYdetritus, :)  	= temp_feces + temp_senescence;                             % paste detritus rows back into BigMatrix; (2D matrix: num_grps X num_grps)
% -------------------------------------------------------------------------
...
temp_metabolism                                 = repmat(ConsumptionBudget_wCannibalism(2, :), [num_nutrients, 1]);      % (2D matrix: num_nutrients X num_grps)
temp_metabolism                                 = temp_metabolism .* fate_metabolism;                       % redistribute metabolism among nutrient rows as defined by fate_metabolism; (2D matrix: num_ANYdetritus X num_grps)
BigMatrix_wCannibalism(looky_nutrients, :)                   = temp_metabolism;                                          % paste nutrient rows back into BigMatrix; (2D matrix: num_grps X num_grps)
% -------------------------------------------------------------------------
...
BioenergeticBudget_checksum                     = round2(sum(BioenergeticBudget_wCannibalism), 7); % (horizontal vector: 1 X num_grps)
looky_error                                     = find(BioenergeticBudget_checksum ~= 1);
if ~isempty(looky_error)
    disp(['      -->WARNING in ' fname_ECOfunction ': BioenergeticBudget_wCannibalism does not sum to 1 (confirm non-zero em) at grps: ' num2str(looky_error)])
end

ProductionBudget_checksum                       = round2(sum(ProductionBudget_wCannibalism), 7); % (horizontal vector: 1 X num_grps)
looky_error                                     = find(ProductionBudget_checksum ~= 1);
if ~isempty(looky_error)
    disp(['      -->WARNING in ' fname_ECOfunction ': ProductionBudget_wCannibalism does not sum to 1 at grps: ' num2str(looky_error)])
end

EnergyBudget_checksum                        	= round2((sum(BigMatrix_wCannibalism) + (cb_ba' + cb_em')), 7); % (horizontal vector: 1 X num_grps); NOTE: ba & em terms are not included in EnergyBudget
looky_error                                     = find(EnergyBudget_checksum ~= 1);
if ~isempty(looky_error)
    disp(['      -->WARNING in ' fname_ECOfunction ': EnergyBudget_wCannibalism does not sum to 1 at grp(s): ' num2str(looky_error)])
end
% *************************************************************************
...
sum_predation                         	= sum(BigMatrix_wCannibalism(looky_livingANDfleets, :), 1); % (horizontal vector: 1 X num_grps)
fate_predation_wCannibalism             = BigMatrix_wCannibalism(looky_livingANDfleets, :) ./ repmat(sum_predation, [num_livingANDfleets, 1]); % (2D matrix: num_livingANDfleets X num_grps)
fate_predation_wCannibalism(isnan(fate_predation_wCannibalism))	= 0; % correct div/0 errors
% *************************************************************************
...
SINGLE_ECO.BioenergeticBudget_wCannibalism	= BioenergeticBudget_wCannibalism;   % (2D matrix: 3 X num_grps)
%                                   1) feces
%                                   2) metabolism
%                                   3) production
SINGLE_ECO.ProductionBudget                 = ProductionBudget; % (2D matrix: 5 X num_grps)
SINGLE_ECO.ProductionBudget_wCannibalism	= ProductionBudget_wCannibalism; % (2D matrix: 5 X num_grps)
%                                   1) eggs (reproduction)
%                                   2) predation
%                                   3) senescence
%                                   4) ba (biomass accumulation)
...
SINGLE_ECO.ConsumptionBudget_wCannibalism	= ConsumptionBudget_wCannibalism; % (2D matrix: 7 X num_grps)
%                                   1) feces
%                                   2) metabolism
%                                   3) eggs (reproduction)
%                                   4) predation
...
SINGLE_ECO.EnergyBudget_wCannibalism	= BigMatrix_wCannibalism; % (2D matrix: num_grps X num_grps)
% -------------------------------------------------------------------------
...
SINGLE_ECO.fate_predation_wCannibalism	= fate_predation_wCannibalism;           % (2D matrix: num_livingANDfleets X num_grps)
% -------------------------------------------------------------------------


% step 11c: ecotrophic efficiency terms -----------------------------------
SINGLE_ECO.ee_eggs                          = ee_eggs_NoCannibalism;              % (vertical vector: num_grps X 1)
SINGLE_ECO.ee_predation                     = ee_predation_NoCannibalism;         % (vertical vector: num_grps X 1)
SINGLE_ECO.ee_ba                            = ee_ba_NoCannibalism;                % (vertical vector: num_grps X 1)
SINGLE_ECO.ee_em                            = ee_em_NoCannibalism;                % (vertical vector: num_grps X 1)
SINGLE_ECO.ee                               = ee_NoCannibalism;                   % (vertical vector: num_grps X 1)
SINGLE_ECO.TransferEfficiency               = TransferEfficiency_NoCannibalism;	% (vertical vector: num_grps X 1)

SINGLE_ECO.ee_eggs_wCannibalism          	= ee_eggs;              % (vertical vector: num_grps X 1)
SINGLE_ECO.ee_predation_wCannibalism     	= ee_predation;         % (vertical vector: num_grps X 1)
SINGLE_ECO.ee_ba_wCannibalism             	= ee_ba;                % (vertical vector: num_grps X 1)
SINGLE_ECO.ee_em_wCannibalism             	= ee_em;                % (vertical vector: num_grps X 1)
SINGLE_ECO.ee_wCannibalism                	= ee;                   % (vertical vector: num_grps X 1)
SINGLE_ECO.TransferEfficiency_wCannibalism	= TransferEfficiency;	% (vertical vector: num_grps X 1)
% -------------------------------------------------------------------------


% step 11d: physiology parameters -----------------------------------------
SINGLE_ECO.ae                       = ae; % (vertical vector: num_grps X 1)
SINGLE_ECO.pb                       = pb_NoCannibalism; % (vertical vector: num_grps X 1)
SINGLE_ECO.qb                       = qb; % (vertical vector: num_grps X 1)
SINGLE_ECO.pq                       = pq_NoCannibalism; % (vertical vector: num_grps X 1)

SINGLE_ECO.pb_wCannibalism        	= pb; % (vertical vector: num_grps X 1)
SINGLE_ECO.pq_wCannibalism        	= pq; % (vertical vector: num_grps X 1)
% -------------------------------------------------------------------------
...
SINGLE_ECO.DIET                                 = DIET;                 % (2D matrix: num_grps X num_grps)
