function [DIET_MC, net_FOOTPRINT_array_MC, gross_FOOTPRINT_array_MC, REACH_array_MC, FOOTPRINT_trace_MC, REACH_trace_MC, FOOTPRINT_system_MC, REACH_system_MC] = ...
    f_FootprintReach_09252023(ECOTRAN, ECOTRAN_MC, PhysicalLossFraction, TraceGroup)
%
...
%
%       net_FOOTPRINT_array_MC      - net "FOOTPRINT" of each TraceGroup = consumer (rows) upon each producer (columns)
%                                   - does NOT include metabolic, feces, and other metabolism losses at each trophic step (not the same as PPR)
...
%
%       gross_FOOTPRINT_array_MC	- net "FOOTPRINT" of each TraceGroup = consumer (rows) upon each producer (columns)
%                                   - DOES include metabolic, feces, and other metabolism losses at each trophic step (if you look just at footprint on primary producers, this should be the same as PPR)
%                                   - the fraction of each producer group's production flowing to each consumer = TraceGroup
%                                   - (3D matrix: num_grps (consumer) X num_grps (producer) X num_MC)
%                                   - NOTE: use for web plotting footprint box colors relative to TraceGroup
%                                   - NOTE: does NOT include import diet.
...
%                           - (vertical vector: num_MC X 1)
%
% revision date: 9-24-2023
%       6/8/2022    allows for detritus-only drivers for deep-water boxes
%       2/12/2023   only cleaned up notes
%       3/9/2023    allowed for gross footpritn option that includes losses to metabolism, feces, and other mortality at each trophic step
%       9/19/2023	re-coded gross footprint calculation to remove recycling feedback loops (necessary to prevent footprints > 100%)
%       9/23/2023	debugging bad matrix precision error in new recycling feedback correction code
%       9/24/2023   Tophic Matrix corrected for import diet
%       9/25/2023   for footprint calcs: deactivate detritus uptake (except for flow to other detritus pools); this reduces recyclng pathways that lead to overestimate of footprint
...
num_MC                      = 1; % over-write num_MC to only process the "type" model
% -------------------------------------------------------------------------
...
net_FOOTPRINT_array_MC      = zeros(num_grps, num_grps, num_MC); % initialize; (3D matrix: num_grps (consumers) X num_grps (producers) X num_MC); NOTE: does NOT include import diet
gross_FOOTPRINT_array_MC	= zeros(num_grps, num_grps, num_MC); % initialize; (3D matrix: num_grps (consumers) X num_grps (producers) X num_MC); NOTE: does NOT include import diet
...
for MC_loop = 1:num_MC
    ...
    current_senescence          = current_ConsumptionBudget(5, :); % (horizontal vector: 1 X num_grps)
    current_fate_senescence     = ECOTRAN_MC.fate_senescence(:, :, MC_loop); % (2D matrix: num_detritus X num_grps (producers))
    % *********************************************************************
    ...
    working_EnergyBudget([looky_micrograzers; looky_consumerANDfleets], looky_detritus) = 0; % turn off detritivory (9/25/23)
    
	current_TransferEfficiency(looky_terminalBnthDetr)          = 0.99; % footprints on terminal detritus will be underestimated by 1-TE = 0.01%.
    % ---------------------------------------------------------------------
    
    
	% step 4b: calculate net_FOOTPRINT_array ----------------------------------
	%          NOTE: use to calculate not footprint (does NOT include metabolic, feces, and senescence losses at each trophic step)
    %          Drive the model with 1 unit of each functional group, individually
	net_footprint_EnergyBudget              = working_EnergyBudget;
	current_net_FOOTPRINT_array            	= zeros(num_grps, num_grps); % re-initialize current_net_FOOTPRINT_array each Monte Carlo loop; (2D matrix: num_grps (consumer) X num_grps (producer))
    ...
    for grp_loop = 1:num_grps
        ...
        q_current_grp                               = f_WebProductivity_03272019(current_TransferEfficiency, net_footprint_EnergyBudget, InputProductionVector, PhysicalLossFraction); % production of each model group (rows) when driven by each current_grp (clms); (vertical vector: num_grps X 1)
        q_current_grp(current_grp)                  = 0; % set footprint of current_grp on self to ZERO
        current_net_FOOTPRINT_array(:, grp_loop)	= q_current_grp; % (2D matrix: num_grps (consumers) X num_grps (producers))
    end % (grp_loop)
    
    net_FOOTPRINT_array_MC(:, :, MC_loop)	= current_net_FOOTPRINT_array; % fraction of each producer group's production flowing to each consumer; (3D matrix: num_grps (consumer) X num_grps (producer) X num_MC)
    % ---------------------------------------------------------------------
    
    
	% step 4c: calculate gross_FOOTPRINT_array ----------------------------
    %          NOTE: use to calculate gross footprint (DOES include metabolic, feces, and senescence losses at each trophic step)
    %          Drive the model with 1 unit of each functional group, individually
    
    gross_footprint_EnergyBudget                    = working_EnergyBudget; % redefine gross_footprint_EnergyBudget with original value
    
    % put senescence back into gross_footprint_EnergyBudget (need flow to guarantee some flow out of matrix or else the matrix cannot be driven)
    current_senescence_repmat                       = repmat(current_senescence, [num_detritus, 1]); % (2D matrix: num_detritus X num_grps (producers))
    gross_footprint_EnergyBudget(looky_detritus, :)	= current_senescence_repmat .* current_fate_senescence; % (2D matrix: num_grps (consumers) X num_grps (producers))
    
    % re-normalize gross_footprint_EnergyBudget to remove losses from metabolism & feces
    OriginalSum_EnergyBudget                = sum(current_EnergyBudget, 1);
    OriginalSum_EnergyBudget_repmat         = repmat(OriginalSum_EnergyBudget, [num_grps, 1]); % (2D matrix: num_grps (consumer) X num_grps (producer))

    sum_gross_footprint_EnergyBudget      	= sum(gross_footprint_EnergyBudget, 1);
    sum_gross_footprint_EnergyBudget_repmat	= repmat(sum_gross_footprint_EnergyBudget, [num_grps, 1]); % (2D matrix: num_grps (consumer) X num_grps (producer))

    gross_footprint_EnergyBudget        	= gross_footprint_EnergyBudget ./ sum_gross_footprint_EnergyBudget_repmat; % (2D matrix: num_grps (consumer) X num_grps (producer))
    gross_footprint_EnergyBudget(isnan(gross_footprint_EnergyBudget)) = 0; % correct for div/0 errors
    gross_footprint_EnergyBudget(isinf(gross_footprint_EnergyBudget)) = 0; % correct for div/0 errors

    gross_footprint_EnergyBudget        	= gross_footprint_EnergyBudget .* OriginalSum_EnergyBudget_repmat; % this 2nd multiplication is only in case the original column sums were not equal to 1
    
	current_gross_FOOTPRINT_array       	= zeros(num_grps, num_grps); % initialize current_gross_FOOTPRINT_array each Monte Carlo loop; (2D matrix: num_grps (consumer) X num_grps (producer))
    
    
    % this code DOES remove trophic cycles back into the target consumer
    % drive food web consecutively with each group as a producer
    for producer_loop = 1:num_grps
        current_producer                        	= producer_loop; % current group as producer
        InputProductionVector                       = zeros(1, num_grps); % re-initialize for each loop iteration; (horizontal vector: 1 X num_grps)
        InputProductionVector(current_producer)     = 1; % drive model with 1 unit of current_producer; (horizontal vector: 1 X num_grps)
        
        % to prevent footprints exceeding 100% of driver production, turn off all recycling loops from each individual consumer group (one-by-one)
        q_current_producer                          = zeros(num_grps, 1); % initialize; (vertical vector: num_grps X 1)
        for consumer_loop = 1:num_grps
            
            current_consumer                    = consumer_loop; % current consumer group
            HigherTLconsumer_EnergyBudget       = gross_footprint_EnergyBudget; % re-set the TrophicMatrix each loop
            
            looky_HigherTLconsumer              = find(HigherTLconsumer_EnergyBudget(:, current_consumer) > 0);
            HigherTLconsumer_EnergyBudget(looky_HigherTLconsumer, current_consumer) = 0; % deactivate higher trophic level consumers of the current consumer group (stop recycling feedback pathways)
        
            q_vector                            = f_WebProductivity_03272019(current_TransferEfficiency, HigherTLconsumer_EnergyBudget, InputProductionVector, PhysicalLossFraction); % production of each model group (rows) when driven by each current_producer (clms); (vertical vector: num_grps X 1)
            q_current_consumer                  = q_vector(current_consumer); % q of current_consumer (mmoles N/m3/d); (scaler)
            q_current_producer(consumer_loop)	= q_current_consumer;
            
        end % q_current_producer
        
        q_current_producer(current_producer)           	= 0; % set footprint of current_producer on self to ZERO
        current_gross_FOOTPRINT_array(:, producer_loop)	= q_current_producer; % (2D matrix: num_grps (consumers) X num_grps (producers))
        
    end % (producer_loop)
    
    
% %     % this code does NOT remove trophic cycles
% %     for grp_loop = 1:num_grps
% %         current_grp                             = grp_loop; % current group as producer
% %         InputProductionVector                   = zeros(1, num_grps); % re-initialize for each loop iteration; (horizontal vector: 1 X num_grps)
% %         InputProductionVector(current_grp)      = 1; % drive model with 1 unit of current_grp; (horizontal vector: 1 X num_grps)
% %         q_current_grp                           = f_WebProductivity_03272019(current_TransferEfficiency, gross_footprint_EnergyBudget, InputProductionVector, PhysicalLossFraction); % production of each model group (rows) when driven by each current_grp (clms); (2D matrix: num_grps X num_grps)
% %         q_current_grp(current_grp)           	= 0; % set footprint of current_grp on self to ZERO
% %         current_gross_FOOTPRINT_array(:, grp_loop)	= q_current_grp; % (2D matrix: num_grps (consumers) X num_grps (producers))
% %     end % (grp_loop)
    
    gross_FOOTPRINT_array_MC(:, :, MC_loop)	= current_gross_FOOTPRINT_array; % fraction of each producer group's production flowing to each consumer; (3D matrix: num_grps (consumer) X num_grps (producer) X num_MC)
    % *********************************************************************
	...
	working_EnergyBudget                     	= current_EnergyBudget; % redefine working_EnergyBudget from original current_EnergyBudget from current Monte Carlo loop
    ...
    if ee_terminalBnthDetr == 0
        ee_terminalBnthDetr = 0.01; % added for SPF ECOBASE models that have dummey terminal benthic detritus groups
    end
    ...
    % step 5c: calculate domestic consumption (q) of each group -----------
    % when driven by NO3 -----
    InputProductionVector                       = zeros(1, num_grps); % re-initialize for each loop iteration; (horizontal vector: 1 X num_grps)
	...
	% when driven by detritus -----
    InputProductionVector                       = zeros(1, num_grps); % re-initialize for each loop iteration; (horizontal vector: 1 X num_grps)
    InputProductionVector(looky_detritus)       = 100 / length(looky_detritus); % 100 units of all detritus groups to drive the model
	InputProductionVector(isnan(InputProductionVector)) = 0; % set div/0 errors to 0
    q_domestic_detritus                         = f_WebProductivity_03272019(current_TransferEfficiency, working_EnergyBudget, InputProductionVector, PhysicalLossFraction); % total consumption q by each model group; (mmoles N/m3/d); (vertical vector: num_grps X 1)
    
    % choose the detritus driver ONLY IF there is no production when driven by NO3 (for boxes that have no primary production)
    temp_q_domestic                             = q_domestic;
    temp_q_domestic(looky_NO3)                  = 0; % zero out driver
    temp_q_domestic_detritus                    = q_domestic_detritus;
    temp_q_domestic_detritus(looky_detritus)	= 0; % zero out drivers
    
    sum_q_NO3                                   = sum(temp_q_domestic);
    sum_q_detritus                              = sum(temp_q_domestic_detritus);
    
    if sum_q_NO3 == 0
        q_domestic = q_domestic_detritus;
    end
    % ---------------------------------------------------------------------
    ...
    % calculate domestic consumption (q) & production (p) of each group 
    % when driven by NO3 -----
	...
	InputProductionVector(isnan(InputProductionVector)) = 0; % set div/0 errors to 0
    q_domestic_detritus                   	= f_WebProductivity_03272019(current_TransferEfficiency, working_EnergyBudget, InputProductionVector, PhysicalLossFraction); % total consumption q by each model group; (mmoles N/m3/d); (vertical vector: num_grps X 1)
    p_domestic_detritus                    	= q_domestic_detritus' .* current_pq; % production rates of all groups when driven with 100 units of NO3; (horizontal vector: 1 X num_grps); NOTE transpose
    ...
    if sum_q_NO3 == 0
        q_domestic	= q_domestic_detritus;
        p_domestic	= p_domestic_detritus;
    end
    % ---------------------------------------------------------------------
...
%     production_CONSUMER([looky_nutrients; looky_detritus]) = []; % remove non-producer group rows (nutrients, detritus, import)
    ...
    %          NOTE: based on NET FOOTPRINT
    FOOTPRINT_array_PRODUCER            = current_net_FOOTPRINT_array; % (2D matrix: num_grps (consumers) X num_grps (producers))
