function [EwEResult, PEDIGREE] = f_AggregateBiologicalModel_05132022(dat)
...
%           5/13/2022 prevented NaN in calculations of pb, qb, & pq
...
for TrophicGrp_loop = 1:num_AggGrps
    ...
    % correct div/0 error NaNs
	agg_pb(isnan(agg_pb))	= 0; % (vertical vector: num_AggGrps X 1)
	agg_qb(isnan(agg_qb))	= 0; % (vertical vector: num_AggGrps X 1)
	agg_pq(isnan(agg_pq))	= 0; % (vertical vector: num_AggGrps X 1)
	agg_TL(isnan(agg_TL))	= 0; % (vertical vector: num_AggGrps X 1)
	agg_ae(isnan(agg_ae))	= 0; % (vertical vector: num_AggGrps X 1)
    % ---------------------------------------------------------------------
...
% filter out div/0 NaNs
agg_ProductionLossScaler(isnan(agg_ProductionLossScaler))               = 0;
agg_RetentionScaler(isnan(agg_RetentionScaler))                         = 0;
agg_FunctionalResponseParams(isnan(agg_FunctionalResponseParams))       = 0; % (2D matrix: num_currentAgg X 4 (preagg))
% agg_FunctionalResponse_matrix(isnan(agg_FunctionalResponse_matrix))     = 0; % (2D matrix: num_currentAgg X num_E2Egrps (preagg)) (FFF this matrix not yet used but will need a lateral aggregation); QQQ deactivated 2/5/2021
...
agg_pb(isnan(agg_pb))               = 0; % correct div/0 error NaNs
% -------------------------------------------------------------------------
...
agg_pq(isnan(agg_pq))               = 0; % correct div/0 error NaNs
agg_ae(isnan(agg_ae))               = 0; % correct div/0 error NaNs
% -------------------------------------------------------------------------
...
agg_qb(isnan(agg_qb))               = 0; % correct div/0 error NaNs
% -------------------------------------------------------------------------
...
EcotrophicEfficiency            = f_calcEE_05122022(randomEwE); % ee terms are fractions of production
