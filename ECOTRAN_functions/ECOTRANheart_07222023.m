function [ECOTRAN] = ECOTRANheart_07222023(EwEResult, MonteCarloStore)
...
%
% calls:
%       f_ECOfunction_07212023                      returns a single ECOTRAN model for 1 "type" EwE model or 1 MonteCarlo EwE model
...
%       5/13/2022 cleared up NaNs in ProductionBudget, ConsumptionBudget, & TransferEfficiency
%       7/22/2023 return terms with and without cannbalism (for MTI)
...
ECOTRAN.fate_predation_wCannibalism   	= zeros(num_livingANDfleets, num_grps, num_MC); % (3D matrix: num_livingANDfleets X num_grps X num_MC)
...
ECOTRAN.EnergyBudget_wCannibalism     	= zeros(num_grps, num_grps, num_MC);	% (3D matrix: num_grps X num_grps X num_MC)
...
ECOTRAN.DIET                                = zeros(num_grps, num_grps, num_MC);	% (3D matrix: num_grps X num_grps X num_MC); DIET with cannibalism
...
%     [SINGLE_ECO]                        = f_ECOfunction_05132022(ModelDefinitions, CurrentModel); % returns a single ECOTRAN model for 1 "type" EwE model or 1 MonteCarlo EwE model
	[SINGLE_ECO]                        = f_ECOfunction_07212023(ModelDefinitions, CurrentModel); % returns a single ECOTRAN model for 1 "type" EwE model or 1 MonteCarlo EwE model
    ...
    ECOTRAN.EnergyBudget_wCannibalism(1:num_grps, 1:num_grps, layer_loop)	= SINGLE_ECO.EnergyBudget_wCannibalism;          % (3D matrix: num_grps X num_grps X num_MC)
    ...
    ECOTRAN.fate_predation_wCannibalism(1:num_livingANDfleets, 1:num_grps, layer_loop)	= SINGLE_ECO.fate_predation_wCannibalism;        % (2D matrix: num_livingANDfleets X num_grps)
    ...
    ECOTRAN.DIET(1:num_grps, 1:num_grps, layer_loop)                        = SINGLE_ECO.DIET;                                  % (3D matrix: num_grps X num_grps X num_MC)
